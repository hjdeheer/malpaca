...
def connlevel_sequence(metadata, mapping):
    ...
    finalClusterSummary(final_clusters, values, inv_mapping)
...
def finalClusterSummary(finalClusters, values, inv_mapping):
    for n, cluster in finalClusters.items():
        summary = {'labels': set(), 'total': thresh * len(cluster), 'malicious': 0, 'benign': 0, 'percentage': 0}
        for connectionNumber in cluster:
            updateSummary(summary, values[connectionNumber][:thresh], inv_mapping[connectionNumber])
        percentage = summary['percentage']

        if percentage > 0:
            print(
                f"cluster {n} is {round(percentage, 2)}% malicious, contains following labels: {','.join(summary['labels'])}, connections: {len(cluster)}")
        else:
            print(f"cluster {n} does not contain any malicious packages, connections: {len(cluster)}")


def updateSummary(summary: dict, packages: list, connection: str):
    percentage = summary['percentage']
...
def readfolder():
    ...
    for f in files:
        ...
        labels = readLabeled(f)
        connections = readpcap(f, labels)

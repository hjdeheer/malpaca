...
# @profile
def connlevel_sequence(metadata, mapping, connsLabeled):
    ...
    for n, clus in final_clusters.items():
        clustersize = len(clus)
        totalPercent = 0
        # print "cluster numbeR: " + str(n)
        for idx, el in enumerate([inv_mapping[x] for x in clus]):

            #Get the labeling list of all the packets send by this connection and calculate percentage of malware
            if el in connsLabeled:
                percent = percentMalware(connsLabeled, el, thresh)
            ...
            outfile.write(
                str(n) + "," + str(mapping[el]) + "," + str(final_probs[n][idx]) + "," + str(classname) + "," + str(
                    filename) + "," + ip[0] + "," + ip[1] + "\n")
        totalPercent = (totalPercent / clustersize)
        print("Cluster " + str(n) + " contains " + str(round(totalPercent, 2)) + "% malicious behaviour!")
    outfile.close()
    print("-------------------------------------")
    # Making tree
    ...
    for filename in files:
        for line in open(filename, 'r'):
            ...
            if len(fields) == 21:
                ...
                destIp = fields[4]
                destPort = fields[5]
                key = os.path.basename(filename).replace("labeled", "pcap") + sourceIp + "->" + destIp
                reverseKey = os.path.basename(filename).replace("labeled", "pcap") + destIp + "->" + sourceIp
                labeling = fields[20]
                labeling = labeling.replace("-", "").strip(" ").strip(" \n").split("   ")
                isMalicious = None
                label = None
                ...
                if labeling[0] == "Benign":
                    ...
                    isMalicious = True
                    label = labeling[1]
                conn = ConnectionData(sourcePort, destPort, isMalicious, label)
                reverseConn = ConnectionData(destPort, sourcePort, isMalicious, label)
                #If connection not in dict, add a list place holder and add the connection obj
                if key not in connsLabeled:
                    connsLabeled[key] = []
                    connsLabeled[key].append(conn)
                else:
                    connsLabeled.get(key).append(conn)
                if reverseKey not in connsLabeled:
                    connsLabeled[reverseKey] = []
                    connsLabeled[reverseKey].append(reverseConn)
                else:
                    connsLabeled.get(reverseKey).append(reverseConn)
    return connsLabeled

def percentMalware(connsLabeled, key, thresh):
    length = min(thresh, len(connsLabeled[key]))
    counter = 0
    max = 0
    for packet in connsLabeled[key]:
        #Break if more than 20 packets
        if max >= thresh:
            break
        if packet.isMalicious:
            print("ALERT! Connection " + key + "(Source: " + str(packet.sourcePort)  + ", " + str(packet.destPort) + ") contains " + packet.label  + " as labeled malicious behaviour")
            counter += 1
        max += 1
    return (counter / length) * 100

    # for key, value in connsLabeled.items():
    #     ratio = 0
    #     if "8.8.8.8" in key:
    #         print(key)
    #         for packet in value:
...
def readfolder():
    #Reads the labeled pcaps
    connsLabeled = readLabeled()
    fno = 0
    meta = {}
    mapping = {}
    ...
    connlevel_sequence(meta, mapping, connsLabeled)
